...
The halving is the same box filter that builds the mip chain, so a 4096 texture is the 8192 texture's first mip level exactly. That is why the default costs so little: the 800x800 render the e2e case checks is byte-identical at 8192 and 4096, and at 2048 the sampled land and ocean pixels move by at most 1/255. Anything that renders the globe larger than a few hundred pixels across will show the difference; the settings window is where to change it back.
