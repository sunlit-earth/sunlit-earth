...
sunlit-earth/
  crates/
    sunlit-core/     # headless: no Slint, no window, no event loop
      ...
      src/scene/     # camera, sky state (astronomy FFI), sun reference path, sun occlusion, datetime
...
Submodules: `gpu_setup` (construction, pipelines, render targets), `render_pass` (uniform encoding, pass encoding, `Overlays::select`, `Stars::select`, `Sun::select`, `read_texture_rgba8`), `textures` (slots, mailbox draining, mipmapped upload, `downsample_2x`), `texture_routing` (which bind group and blend mode), `frame` (`FrameState` dirty check), `uniforms` (the 400-byte `#[repr(C)]` struct).
