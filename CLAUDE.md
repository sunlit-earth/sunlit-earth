...
**Environment knobs**, all opt-in and all falling back to the compiled-in behavior when unset or blank: `SUNLIT_EARTH_TEXTURES` (textures directory), `SUNLIT_EARTH_NO_CLOUDS` (skip the cloud fetcher), `SUNLIT_EARTH_CLOUD_URL`, `SUNLIT_EARTH_CLOUD_POLL_SECS`, `SUNLIT_EARTH_CACHE_DIR` (cloud cache location), `SUNLIT_EARTH_CONFIG` (config file path), `SUNLIT_EARTH_SYNC_LOG` (synchronous stderr logging), `RUST_LOG`. The e2e tests pass `SUNLIT_EARTH_CONFIG` on every spawn so they never read or write the developer's real settings; without it a test run replaces the desktop wallpaper whenever auto-refresh is enabled in the real config.
...
- No unbounded channel crosses a thread boundary. Every producer/consumer boundary is bounded or latest-value, and the choice is justified at the declaration site. Decoded pixel buffers are never parked in queues or long-lived structs, only compressed bytes or GPU textures. Every background producer must name its consumer and the condition under which the consumer runs; if that condition is not "always", the design is wrong. These rules come from the 7.3 GB tray-mode leak, see `docs/retrospective-2026-08.md` sections 4.1 and 8.2.
