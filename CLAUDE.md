...
**The Windows golden image cannot be built on a Windows host.** Packer installs it under QEMU, QEMU there uses WHPX, and a WHPX guest with more than one vCPU does not survive the reboot Windows Setup performs after copying its files: `WHPX: Unexpected VP exit code 4`, unrecoverable. One vCPU survives it and Windows 11 Setup refuses to install on one core; `kernel-irqchip=off` keeps the vCPUs and stops the guest booting at all. `qemu_cannot_install_windows` in `commands::build_image` holds the measurements and warns before a build starts, and `docs/vm-setup.md` has the full account. A Linux host builds the image normally. The Linux image and guest work on a Windows host today.

`vm build-image` watches its guest while Packer builds, because Packer says nothing between the boot prompt and the guest's first SSH answer (`commands::build_watch`). One QMP connection presses the installer's boot key and then prints a line a minute: what the output disk holds and how fast it is growing, whether the guest's screen is changing, and what QEMU says the machine is doing. A guest QEMU has stopped is reported rather than waited out: the build starts it again once, and if that changes nothing it ends the guest so the build fails now instead of at Packer's two-hour SSH timeout. The build directory keeps `screen.png`, the guest's last screen as QEMU encoded it, and `packer.log`, which is where QEMU's stderr ends up.
