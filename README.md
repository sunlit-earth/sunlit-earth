...
cargo xtask bake licenses          # walk the dependency tree into the two third-party notices
...
GPL-3.0-or-later, as declared in `Cargo.toml`; the full text is in [LICENSE](LICENSE). Slint is used under its GPLv3 option, and Astronomy Engine is MIT.

The credits are in [assets/ATTRIBUTION.md](assets/ATTRIBUTION.md), which is also what the About window's attributions tab renders. Two files go further: [assets/THIRD-PARTY-LICENSES.md](assets/THIRD-PARTY-LICENSES.md) carries the license of every crate the program depends on and the full text of each of those licenses, taken from the canonical SPDX texts committed under `assets/licenses/`, and is the one of the three that ships beside the binary, at the top level of the release archive, generated by `cargo xtask bake licenses` along with the crate list the window's third tab shows, and [textures/PROVENANCE.md](textures/PROVENANCE.md) is the full record for the imagery, with source IDs, file hashes and download dates. The star catalog's own notice sits beside the blob it belongs to, in `crates/sunlit-core/src/assets/stars/ATTRIBUTION.md`.
