...
- **Commands**: `UpdateParams`, `SetPreviewSize`, `SetPreviewEnabled`, `RenderWallpaperNow`, `RenderToFile`, `ExportPixels`, `SetTextureResolution`, `SetDisplayPlan`, `ReportMemory`, `SetAutoRefresh`, `Poke`, `Shutdown`. `SetDisplayPlan` carries the display mode and the anchor screen, which are settings rather than shader parameters and so travel on their own command, exactly as `SetTextureResolution` does.
