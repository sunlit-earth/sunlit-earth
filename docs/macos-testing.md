...
**The bundle.** Unzip, move `Sunlit Earth.app` from the folder that gives to Applications, double-click it. The first attempt gives a dialog saying macOS could not verify that the app is free of malware. Open System Settings, go to Privacy & Security, scroll to the bottom, and choose Open Anyway for Sunlit Earth. On macOS 26 that asks for an administrator password; on 15 it does not. `xattr -dr com.apple.quarantine "/Applications/Sunlit Earth.app"` in Terminal does the same thing by removing the attribute the browser set, and is worth trying if the System Settings route does not appear.
