...
- M1, both halves: `TextureMailbox::post` keeps a parked message whose generation is newer than the arrival's, so the clobber cannot happen, and `process_decoded_textures` clears `loading` when it discards, so any ordering that still reaches the discard heals with one redundant decode. Decision 5's sentence is now true as written. Mailbox unit tests cover the ordering; removing either half fails a test.
- m1: the quick-succession test's comment now says what it does test, and `a_stale_decode_arriving_after_a_switch_is_never_applied` is the case for the ordering. It uses a new `EngineConfig::mailbox`, injected the way the clock and the cloud source are, because a decode still running when the resolution changes cannot be arranged by waiting. Its payload is solid black and its second half asserts that a discarded arrival produces no frame at all, which is what makes it fail if the fix is removed.
