...
5. **Success criterion 4's "the very next frame" is read as "no frame ever shows the old width".** A reload is a file decode on a background thread, so there is no arrangement in which the frame immediately after the command is drawn from textures that do not exist yet; Decision 4 chooses that gap deliberately by purging before spawning the loads. What the implementation guarantees is the part that can be guaranteed: no frame is ever drawn from the old width's textures (they are destroyed), no frame is drawn from a superseded decode (the generation stamp), and the frames in between show the procedural grid, which is the same fallback a cold start shows. Round 1 found the one place where that fallback is not acceptable, the desktop itself, and a wallpaper update asked for during the gap now waits for it (m6). `a_resolution_switch_reloads_the_textures_in_both_directions` asserts a lit frame after the reload in both directions.

6. **The discard path does not touch the slot's `loading` flag, where round 2's direction was to make the flag carry the in-flight generation and clear it on a match.** Round 1 added a clear there as a second line of defense; m8 found it could put a second 8K decode in flight beside a live one, and the fix asked for was a narrower clear. Writing it showed the narrow version can never fire. A post is discarded only when the generation has moved on; the generation moves only in `set_texture_resolution`; and that function purges every file-backed slot in the same call, which clears the flag. So by the time a stale post is drained, its slot's flag was either cleared there or has since been set by the reload, and a generation-matched clear has no case left to match. Carrying a generation in the flag to express a condition that is provably unreachable is worse than the plain flag, so the clear is gone and the invariant is written at both sites instead. This also resolves what round 1 recorded as the fix's second half: the half that heals is the purge's clear, not the discard's, and `a_switch_while_the_first_load_is_running_still_converges` is the test that holds it to that, verified by reverting it.

7. **`EngineConfig::cloud_cache_dir` is now `cache_dir`.** Decision 3 puts the texture cache under the directory the cloud cache uses, and the field is what carries that directory into the engine. Keeping the old name would have meant reading the cloud's field to find where textures go.
...
Disposition: all eight fixed, none declined.

- M1: `TextureMailbox::post` keeps a parked message whose generation is newer than the arrival's, so the clobber cannot happen. Decision 5's sentence is now true as written. Mailbox unit tests cover the ordering, and removing the guard fails one of them. The second half added here, a `loading` clear in the discard path, turned out to be both unnecessary and harmful and was removed in round 2; see that round's record.
- m1: the quick-succession test's comment now says what it does test. The case for the ordering itself was added here and rewritten in round 2, where it was found to arrange the interleaving it was supposed to test against; the round 2 record has what it does now. `EngineConfig::mailbox` dates from here, injected the way the clock and the cloud source are, because a decode still running when the resolution changes cannot be arranged by waiting.
- m2, m5's two sentences, m7: corrected.
...
Disposition: all six addressed. m8 was fixed by deleting the line rather than narrowing it, which also settles m1's second half; that is departure 7.

- m1 (carried): three engine cases now, each one verified by reverting the line it protects and watching it fail. `a_stale_decode_must_not_replace_the_texture_that_superseded_it` parks the reload's own post first and the superseded decode's second, which is the ordering that loses a texture, and it makes the slot terminal first by deleting the file behind it so that nothing the engine does can supply one afterwards: `TexturesReady` can then only fire if the fresh post survived. Without the mailbox guard it times out. `a_stale_arrival_produces_no_frame_at_all` asserts a discarded arrival dirties nothing, and fails if the generation check goes. `a_switch_while_the_first_load_is_running_still_converges` waits for the loading status before switching, so the purge happens with a decode genuinely in flight, and it fails if the purge stops clearing `loading`: that is where the healing half lives, which is what round 1 got wrong about it. The false claims in the round 1 record are corrected above.
- m8: fixed by removing the clear, not by narrowing it to a matching generation. A post is discarded only when the generation has moved on, which happens only in `set_texture_resolution`, which purges every file-backed slot in the same call; so a discarded post's slot has already had its flag cleared there or reset by the reload, and the discard has nothing left to do but harm. Both the invariant and the reason are recorded at the two sites, and the mid-load test is what holds the purge to its half of it.
