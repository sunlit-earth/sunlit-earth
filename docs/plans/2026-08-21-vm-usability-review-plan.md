...
3. **Minor D was fixed by changing `enable_enhanced_session`'s return type rather than by special-casing the target in `hand_over`.** The finding's cheap fix was for `hand_over` to answer false where there is no session to offer, which would have put the Windows check in two places: once in `enable_enhanced_session`, where it decides whether to run anything at all, and again in `hand_over`, where it would decide what that `Ok` meant. Returning `Result<bool, String>` keeps one place deciding, and it names the distinction the record needs: the `Ok` that means "turned on" and the `Ok` that means "there was nothing to turn on" are different answers, and only the function that knows which guest it is looking at can tell them apart. The signature is public and has one caller.
...
Four fixed, one declined. Two commits on `feat/phase3-vm-orchestration`, all three gates green on the Windows host, no VM booted, built, or torn down.

- **A. Fixed.** `provider::hyperv::CREDENTIAL_DIALOG_CAVEAT` is the one sentence about a console that asks for credentials it should not, and both texts print it: `hyperv::view_note(false)`, which `vm view` prints, and `commands::vm::view_note(Windows, false)`, which `vm smoke --keep` and a failed hand-over print. A test asserts both carry it, that neither handed-over text does, and that a Linux guest's does not.
- **B. Fixed.** The caveat leads with the advice, which does not depend on the cause, and then names all three causes rather than asserting one: an image built before the Remote Desktop Services disable (named as the usual one, with the rebuild command), a hand-over that started the service and then failed before the guest confirmed it, and a guest that is still being installed, which has no golden image behind it. What the dialog wants is no longer discussed at all, because that differs by cause and the advice is to cancel. `docs/vm-setup.md` says the same, hedged the same way, since the guide carried the single-cause sentence too.
- **C. Declined.** `Prepared.staged` says staging ran, and making it say the shortcuts arrived means widening `artifacts::stage`'s return type for both of its callers, so that a convenience the code deliberately does not fail on can be reported one level up. What that buys is exactness in one rare case: `handover::prepare` fails as a unit, so reaching it takes a full disk or a broken `WScript.Shell`, and when it is reached the warning naming the failure is printed on the line immediately above the closing text. Somebody who has just read `warning: the guest would not write the desktop shortcuts` is not misled by the next paragraph describing what would have been on the desktop, and the field's own doc comment states that boundary. The cost is a wider signature and a second fact for two callers to thread through; the benefit is one sentence in a case whose diagnosis is already on screen. Out of proportion, so it stands as documented.
- **D. Fixed.** `handover::enable_enhanced_session` answers whether the guest ends up offering an enhanced session rather than whether it failed, so a target with none to offer answers false without asking the guest anything, and `hand_over` writes that (departure 3). A unit test drives a Linux guest through it with a fake runner that has no response registered, which proves both the answer and that no command was sent.
- **E. Fixed.** `windows_media::download_command` says at the site why it is a bare `PATH` lookup: every fallback `resolve_tool` adds is Windows-only and named for a tool `vm setup` installs there, `curl.exe` is in `System32` beside the `powershell.exe` everything here already runs, and on Linux `resolve_tool` has no fallbacks at all.
