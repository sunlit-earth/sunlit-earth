...
Scope: the range 0304514..c38a9d9, a fresh validator, all three Windows gates re-run green by it (873 passed, 11 ignored, clippy clean after a fresh `cargo clean -p`), criterion 3 reproduced live on the real assets. Verdict: 1 major, 3 minors; the round blocks.
...
Scope: the fix range 71d629f..b8e5e57, the round 2 validator resumed. Verdict: both round 2 fixes verified, m4 by mutation on both of its halves (an unconditional sidecar save fails the new test, and neutralizing that assertion makes the restart half fail on Unchanged against Updated), and the round records checked against the findings they record. 0 new majors, 3 new minors; the round does not block.

- m6: the sidecar was still written before the decode was known to succeed, the one remaining door to the M1 symptom. A body that downloads and writes fine but does not decode, a captive portal answering 200 with an error page and an `ETag`, left a valid-looking entry on disk and recorded the `ETag` in memory, so the backoff's retry revalidated into a 304 and never retried in substance, and on disk the entry defeated `post_cached` and every later switch back to that variant. Pre-existing, identical at 0304514, but the m4 fix made the invariant load-bearing, and this was the last way to violate it.
- m7: the round 1 record's scope read b77b191..c38a9d9, which as git syntax excludes b77b191, the memory report commit and the largest in the round. The range that round reviewed was 0304514..c38a9d9, which is what its findings prove.
- m8: criterion 6 asks for the gates in WSL as well, and no round record carried the WSL outcome; it lived only in the run's scratch log, and the two known pre-existing WSL failures were recorded nowhere durable.

Disposition: all three fixed by the orchestrator, m6 in 87a1b06 and m7 and m8 in the commit that records this round.

- m6: the poll decodes before it persists anything, so a failure keeps the previous entry and the previous `ETag` and the retry is a real re-download. Mutation-checked: persisting the claim in the decode-failure arm fails the new test on the sidecar recording the bad body's `ETag` over the last good one.
- m7: corrected to 0304514..c38a9d9 above.
- m8: recorded here. At 87a1b06, `cargo test --workspace` in WSL (Ubuntu 22.04, lavapipe): every target green except two xtask cases that compare Windows path separators against `Path::join` output (`provider::hyperv::tests::the_settings_file_is_named_after_the_vm_identifier`, `store::windows_media::tests::the_efi_boot_image_is_required_and_the_bios_one_is_not`), which fail identically at the phase 4 base commit and predate this work. `tests/shading.rs` is additionally flaky under WSL (a wgpu-hal gles/egl BadAccess on roughly one run in five, measured at the same rate at base 0304514 and on the branch); it passed this run. Fixing the two path tests and the flake is recorded as separate work in the roadmap's Bugs and polish list rather than here.
