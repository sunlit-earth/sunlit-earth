...
3. `star_intensity` and `star_mag_limit` round-trip through config save/load and the Slint bridge, have digest rows, and intensity 0 demonstrably skips the draw (no star pixels, engine test).
...
- Cross-adapter golden stability of small sprites. Mitigated by the 2-plus-pixel PSF; if an adapter still disagrees, widen the PSF floor rather than the tolerance.
