...
The transit band is where the analytic approach pays off visibly. While the line of sight to the sun passes through the atmosphere shell (between the solid limb and the Rayleigh radius), the same geometry yields a transit factor that shifts the glare warm and dims it: the ISS-sunrise look, a warm glare kindling at the limb before the white disk breaks free. What shipped in phase B was that factor driving one flat orange tint, and the amendment replaced it with the light path itself: one scalar cannot give a disk a gradient, and the gradient is what the photographs show. The photographic reference is well documented: at grazing incidence the light path through the lower atmosphere is long, blue is scattered out, and what remains is a concentrated orange band with a thin blue line above it. An optional second-order polish, worth trying with screenshots: a forward-scattering boost in `fs_rayleigh` that raises the existing limb glow near the sun's azimuth, which the research shows is the standard cheap rim-term proxy for real scattering. The two effects are independent; the second can land later or never. Tried during phase B on 2026-08-26 and removed: as `(1 + 0.6 * clamp(n_dot_l, 0, 1)^2)` on the in-scattering term it moves the grazing-limb composition by a mean of 0.007 of 255, at most 7, and 0.069 over a crop tight around the limb. The reason it does so little is that `n_dot_l` is the only proxy available inside that shader and the existing day, terminator and night-fade terms already weight the ring by it, so the boost reweights something the shader models rather than adding an effect. Worth revisiting only with a real phase function, which wants the view ray rather than the surface normal. That is what the amendment of 2026-08-29 did, and `fs_rayleigh` has the view ray: the lobe is Henyey-Greenstein in the scattering angle `acos(-dot(sun_dir, v))`, which is forward exactly where the Sun is behind the fragment as seen from the eye, colored by the same limb transmission that colors the Sun's own disk and gated on the fragment being sunlit, with its half-width and its strength as controls. Deleting it now moves the `sunrise_band` reference by a mean of 1.52 with 2.83 percent of its pixels outliers, against the 0.007 the `n_dot_l` version managed. What it costs is one clause the first attempt would not have needed: plain Henyey-Greenstein keeps 3.3 percent of its peak at every backscattering angle, so the floor is subtracted and the remainder renormalized, or every day-side frame gains a warm ring around its whole limb.
...
- **Phase B: the Sun.** The glare pipeline and composition of section 5, the analytic occlusion and transit factors, the three parameters. No assets. Landed 2026-08-26, with two changes worth knowing about here: the occlusion factors live beside the uniform encoder rather than in `SkyState`, because they are a function of the lens and the viewport as well as of the time, and the `fs_rayleigh` forward-scatter boost was tried and dropped. Plan: [2026-08-23-celestial-b-sun-plan.md](2026-08-23-celestial-b-sun-plan.md).
...
- **Backlog, deliberately unplanned:** eclipse rendering (section 6.5); an HDR intermediate target with convolution bloom, which would make star and sun brightness physically grounded instead of artistically mapped and subsume parts of section 5; the Rayleigh forward-scatter boost, which phase B implemented, measured and removed. These need further refinement before any of them gets a plan.

## 12. Open questions

1. Default intensities are taste decisions to make with rendered screenshots during implementation: the star magnitude default (6.5 versus deeper), the sun glare defaults, and whether the lenticular halo gets its own slider. The sun half is settled as of phase B: glare 1.0, rays 0.6, camera mode 0, and the halo stays under the glare parameter with no slider of its own. The Milky Way's is settled as of phase D at 0.5, with the asset carrying no exposure gain of its own. The star magnitude default is still 6.5.
...
3. Resolved 2026-08-27: the layer defaults on at every tier, at intensity 0.5. The tier caps MSAA and the preview width and not textures, so there was no tier-shaped reason to switch it off; what does move the memory is the texture resolution setting, and the 4096 source halves through the existing cache to 10.7 MiB at the narrow end. Phase D, decision 5.
...
5. Added 2026-08-24, resolved 2026-08-27: whether the milky way layer's 4096-wide source is enough resolution under the sky lens. The panorama carries 11.4 texels per degree; a 4K render shows 23.9 screen pixels per degree at the default 140 degree sky and 62.5 at the narrowest 60, so the layer is magnified about two to five and a half times. It is enough. Checked against a render at the narrowest setting, the hardest case the sky lens can produce: no pixelation and no blockiness, and what is visible is Gaia photon noise in the source rather than a resolution limit, which the 8192 variant would carry identically at four times the memory term. Phase D, departure 4.
