...
Medium. Nothing about the shaders, the engine loop or the texture pipeline moves. What moves is the `WallpaperSink` contract, which has three implementations and one test double, and the Windows setter, which gains a COM interface next to the `SystemParametersInfoW` call it has now. A single-monitor session must come out of this byte-identical to what it does today, and that is the first test to write. Amended while implementing: that holds for a landscape screen, which is every configuration this project is regression-tested on, and it deliberately does not hold for a portrait one, where the contain rule below asks for the opposite. Departure 9 has the arithmetic and the reason the rule wins.
...
- The shader clamps `sky_fov`, and where it clamps decides whether an ordinary span keeps its sky. Written here as `[60, 180]`, mirroring the slider; widened to `[60, 330]` while implementing, because nothing happens to a stereographic lens at 180 and what is linear in canvas pixels is `tan(sky_fov / 4)` rather than the angle. See departure 7. At 330 the sky saturates once `W / A.width` passes about eleven for the default 140 degrees, which is a layout nobody has; at 180 two equal monitors side by side were a ratio of 2 and clamped immediately. Where it does clamp, the globe, the atmosphere and the Sun still continue exactly, the sky is drawn at a smaller scale than the anchor alone would have drawn it, and the publish says so. Log it when it clamps.
...
1. **Mode**, a `SettingCombo` naming the three modes: "One screen", "Mirror", "Extend". Departure 11 says why they are not the sentences this line first asked for.
2. **Screen**, a `SettingCombo` whose model comes from Rust: "Primary (automatic)" first, then one row per monitor, labeled like `DP-1  3440x1440  primary` on Linux and `Display 1  2560x1440` on Windows. It is the anchor for every mode, and its hint says so: which screen gets the picture in one-screen mode, and which screen the view is centered on in the other two.
3. **A layout diagram**, a small box drawing the monitor rectangles to scale with the anchor highlighted and each rectangle labeled with its index. The model is `[{ label, x, y, w, h, anchor }]` normalized to 0 to 1 in Rust, so the `.slint` side only multiplies by the box size and the arithmetic stays where it can be tested. This is the part that makes a wrong anchor or an unexpected layout obvious without a second monitor to look at.
...
8. **The GPU span-identity cases live in `tests/engine.rs`, not `tests/render_pipeline.rs`.** The plan names `render_pipeline.rs`, which holds a wgpu device of its own behind a `LazyLock<Mutex<GpuContext>>` and its own copy of the `Uniforms` struct with hand-built matrices. A case written there would have had to rebuild the projection from the derived framing by hand, which would have tested the test's arithmetic rather than the renderer's, and an engine alongside that shared context would be a second device in one process, which is what the one-device-at-a-time rule in CLAUDE.md exists to prevent. In `engine.rs` the case publishes twice through a `RecordingSink`, once in `AcrossScreens` and once in `EveryScreen`, and compares the anchor's crop against its own render: the whole path, `build_wallpaper_job` and `write_uniforms` included, with nothing reimplemented.

9. **A single-monitor session is byte-identical on a landscape screen and deliberately is not on a portrait one.** The Stakes section asks for byte-identity from every single-monitor session; the contain rule under "Different aspect ratios" asks that a screen taller than it is wide be given a lens the globe fits inside. On a portrait screen those are the same demand pointing opposite ways, because what the rule changes is exactly what identity forbids changing, and `build_wallpaper_job` applies `screen_framing` to every screen with no monitor-count condition. So a lone 1080x1920 screen at the default `camera_fov` of 20 renders at 34.8 degrees rather than 20: `2 * atan(tan(10 deg) * 1920 / 1080)` is `2 * atan(0.3135)`, which is 34.81. The contain rule wins, decided by the user on 2026-09-01. It is the plan's own rule; the picture it makes is the one that screen should have had all along, because a fixed vertical field of view ran the globe off both sides of it; and the identity it costs is the identity of an output nobody would defend. What it does not cost is the case all the regression evidence is about: `contain_camera_fov` returns its argument untouched for `width >= height`, so every landscape screen is byte-identical as promised, which is every config anybody has and every guest this suite runs in. The goldens are unmoved too, and for a duller reason: they render directly and never reach this path at all. Both halves are pinned rather than left latent. `one_monitor_is_one_image_at_its_own_size_in_every_mode` now compares the publish against `EngineCommand::ExportPixels`, which is the pre-feature path unaltered (`prepare_export`, then `export_image` with the scene's own parameters), byte for byte in all three modes; `a_portrait_screen_takes_the_contained_lens_instead_of_the_old_one` asserts that a portrait screen is *not* that render and *is* the render at `contain_camera_fov` of the same setting.

10. **KDE reaches every screen, through the scripting API rather than through Plasma's own tool.** The plan put KDE at `OneImage` on the reasoning that per-screen wallpapers need "a new program on `PATH` and an ordering assumption between containment index and monitor; neither is a thing to write blind." Both halves turned out cheaper than that. The program is `dbus-send`, which arrives with `dbus-x11` and is already in the guest image, so it is not a new dependency in any practical sense. The ordering assumption is avoidable rather than unavoidable: Plasma hands out a containment's `screen` index and that screen's geometry, so sorting the containments by geometry and matching them against a placement sorted the same way replaces the assumption with a computation. `Placement::by_position` exists for that, carrying one entry per screen in layout order with a hole where a screen was left alone. The consequence is that one-screen mode works on KDE at all: `plasma-apply-wallpaperimage` writes every containment, so before this it painted every screen with the anchor's picture, which is the opposite of what that mode means. Plasma still has no span mode, and still does not need one, because the span arrives already cut. Unobserved: no two-head guest has run the script, which `docs/roadmap.md` carries.

11. **The mode combo names the modes rather than describing them.** The plan asked for the three "spelled as sentences": "One screen", "Same view on every screen", "One view across all screens". The combo is not wide enough for them and cut the last two off on the machine this ships to, which makes a sentence worse than a word: a truncated description is no easier to read than a term and looks broken besides. They are "One screen", "Mirror" and "Extend", which is what a display settings panel already calls these arrangements, so they read as the established terms rather than as abbreviations of the plan's sentences. The sentences did not disappear; they moved to the row's hint, which is the widget built to hold them and which now spells out all three by name.

## Open questions

1. ~~Is the process per-monitor DPI aware, and is `rcMonitor` therefore physical pixels?~~ Answered by reading winit 0.30.13 and Slint 1.17.1: see departure 3. The app declares `PER_MONITOR_AWARE_V2` itself before enumerating, so `rcMonitor` is physical pixels on every path, headless ones included, and no scaling factor belongs in the layout math. What is still unverified is the behavior that follows from it under real mixed scaling, which is what `sunlit-earth displays` on a two-screen machine is for.
...
3. ~~Does the Plasma containment index line up with the monitor order, and is `dbus-send` an acceptable new dependency for the KDE row?~~ Answered on 2026-09-01, in part. `dbus-send` costs nothing: it comes with `dbus-x11`, which the guest image already carries and which any session running plasmashell has by definition. The index question is answered by not relying on it: Plasma numbers containments by screen index and renumbers them when the layout changes, so the script sorts them by `screenGeometry` and matches that against `Placement::by_position`, which is sorted the same way. KDE is `PerMonitor` as of departure 10. What is still open is the run: no two-head guest has executed the script, so the ordering, the hole that holds an unpainted screen's place, and whether Plasma takes the `file:` URI the config stores are reasoned rather than observed. `docs/roadmap.md` carries that as its own item, with `readConfig` through `evaluateScript` as the read-back to write when someone does it.
