...
The floor is **the parallel dependency compile**, about eighty seconds of sixteen rustc processes at once, and it is over by the time the fat-LTO link starts. The link then runs for three minutes with 2.3 to 2.7 GiB free, which is the opposite of what the plan's risk section expected from the shape the Linux builder's OOM note describes: on this workspace the last few crates holding a lot at once is not the peak, sixteen of them holding a little each is. The control's curve is the same curve shifted up, but not by the same amount everywhere, and the difference is the point: about 1.8 GiB of the extra 2 apart at the plateau, and **0.7 GiB apart at the floor**, 1,411 MiB against 692. So most of a raised ceiling arrives as file cache during the link and only a third of it as headroom at the peak, which is where the question was.
...
Two other things nothing here measures, both of them deliberate. **Decision 2's kill has never fired**: every stop of a healthy Windows guest in this session complied in 7 to 17 seconds, well inside the 60 second grace, so what the record proves is the asking and not the insisting. And **the Linux builder's memory is untouched at 8192**, which the non-goals reserve; the measurement above is about a Windows guest's file cache and says nothing about a fat-LTO link on Ubuntu. Its vCPU count is not untouched, though, and that is worth saying plainly: `builder_cpus` has a floor and no ceiling, so `LinuxBuilder` went from 8 to this host's 16 along with the Windows one while its memory stayed where the non-goals put it. `dist --target linux` was never run here, so doubled parallelism at unchanged memory is untested on the builder whose OOM the docs already describe. `--jobs` is the named lever if a big host ever finds the ceiling.

### Validation round 1

One major and three minors over `4ad4c31..20df52b`. All four addressed; nothing declined.

**Major, a failed resume destroyed the overlay the feature exists to keep.** `vm::resume` reused `boot`'s error branch verbatim, and that branch is `after_failure(.., keep = false)`, which tears the guest down and unlinks everything `run_state_paths` lists, the overlay included. Correct for `boot`, which created that overlay three lines earlier, and exactly wrong for `resume`, whose own doc comment says the overlay is deliberately not recreated. The failure it would have hit is the one the risk section names: decision 2's kill leaves the guest's filesystem unclean, the resumed guest runs a repair pass, `wait_ssh` gives up after `BOOT_TIMEOUT`, and ten minutes of waiting ends by deleting the build directory that was the whole reason the guest was kept. It also broke decision 6 through `builder_session`, since a staging build whose resume hiccupped would have left the builder destroyed rather than as it found it.

`resume` now has its own failure path, `vm::after_failed_resume`, and it destroys nothing. A guest that is up is left up, because powering off a guest that is busy with itself starts that work over, and the message names `vm ssh` and `vm view` to look, `vm stop` to put it back, and `vm down` to give it up on purpose. A guest that never came up, which is the other shape (a port the provider could not bind, say), gets `stopped: true` written back, because a record left saying otherwise would have `vm status` call a resumable builder a crashed one and point at the `vm down` this whole path exists to avoid. Three tests cover it, and the fake provider they use answers `unreachable!("a failed resume destroys nothing")` to `destroy`, so a future edit that reintroduces the teardown dies in the test rather than in someone's builder.

**Minor 1, the Hyper-V stop was not bounded by `SHUTDOWN_GRACE`.** True as reported: `run_script` is `runner.capture` with no timeout and `Stop-VM` without `-AsJob` is synchronous, so the grace only started once the blocking call had already returned. Fixed rather than declined, in the only place it can be fixed, which is inside the script: the request is now `Stop-VM -Force -AsJob | Wait-Job -Timeout <grace>`, and the `wait_for_off` that follows it drops to `Duration::ZERO`, one question rather than a second wait. The turn-off keeps its real poll, because that one is genuinely asynchronous. A test reads the script for both halves. Still unrun on a Windows host, as everything on that side is.

**Minor 2, decision 8 changed the Linux builder too.** Written into "What is not measured here" above.

**Minor 3, `vm up <builder>` and `vm start <builder>` differ.** A paragraph in `docs/vm-setup.md` next to the two commands, and the mechanism in `docs/vm-internals.md` beside `persistence_refusal`.

Also taken: the validator's unraised imprecision about the two memory curves. "Shifted up by about 1.7 GiB" was true of the plateau and not of the floor, and the floor is what decided the question, so both numbers are in the paragraph now.

### After the round: the stop that killed without saying so

Found by using the branch rather than by reviewing it, which is why neither the plan nor the validator caught it. The first `vm up windows` after the builder had been freed compiled cold, staged, and left the builder stopped as decision 6 says. The next `vm up windows` resumed that builder and sat at "waiting for the guest to answer on SSH" until it was interrupted.

`QemuProvider::end` returned one and the same `Ok(Stopped::Stopped)` on two different exits: the QMP request completed inside the grace, or it did not and the process was terminated. The fallback printed nothing. So a builder that was killed at the grace reported the same closing line as one that shut itself down, and the record said `stopped: true` either way. The cost is not the kill, which decision 2 accepts on purpose; it is that the kill was invisible. What a person then meets is a resume that does not answer, with nothing anywhere that says a repair pass is running, which is exactly the symptom that arrived. The code's own comment at that spot had named the cost, calling it "a cost a teardown does not care about and a stop does", and the stop then took it silently. The Hyper-V half already printed a line before turning the power off, so the two halves disagreed, and the one that runs on this host was the quiet one.

`Stopped` gains a third variant, `Killed`, and the old `Stopped::Stopped` becomes `Stopped::ShutDown`, both because three variants made clippy's `enum_variant_names` fire and because the name now has something to be distinguished from. QEMU's `end` returns it whenever it had to terminate and prints `qemu::insist_line` first, which distinguishes a guest that spent the whole grace from a QMP socket that never answered, because those are different faults. Hyper-V's turn-off returns it too. The two callers that keep the overlay say what it costs: `vm::stopped_line` for `vm stop`, and `artifacts::left_stopped_line` for the automatic stop at the end of a build, which is the one nobody watches. `teardown::execute` collapses the two, since it deletes the overlay they differ over. Four tests, all over the pure functions.

This does not make a killed builder rarer, and it is not meant to. Whether a stop after a long build needs more than sixty seconds is a measurement nobody has taken; the grace stays where decision 2 put it, and a kill is now something you can see.
