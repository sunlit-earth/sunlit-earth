...
Implemented with one more shape than that, for the reason in departure 14: where the session has one output rather than two, the case switches that output to another of its modes instead of switching a screen off. Same RandR event, same comparison, same republish; what is left unproven there is only the screen *count* moving. It asks the session which of the two changes it can make, checks that the change took before waiting on anything, and skips with a printed reason where the session put its own layout straight back. Run on 2026-09-03 in the one-head KDE guest: `Virtual-1` went to 5120x2160, the app printed `SIGNAL:displays_changed monitors=1` and published one image for the new rectangle, and the restore produced the same pair again.
