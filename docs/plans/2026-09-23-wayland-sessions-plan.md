...
Round 1, on 3d16926..e318ec8: 0 major, 8 minor, all fixed in 9363f9c (the misplaced test doc comment, the lost line wraps in `desktop.sh`, `desktop_linux.rs` and `app.rs`, the windowing comment's claim about the deferred hide, and an SSH failure reported as a session mismatch) and cb40ae7 (the Windows e2e counts in `platforms.md`, the displaced fw_cfg paragraph in `vm-internals.md`, and the stale and overclaiming roadmap sentences).
