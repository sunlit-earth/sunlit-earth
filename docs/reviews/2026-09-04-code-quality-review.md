...
Date: 2026-09-04. Tree: commit `19312ba` (0.1.0-beta.1). Scope: `crates/sunlit-core` and `crates/sunlit-app`, source and tests, plus the shaders and `ui/main.slint`. Status: findings and recommendations only, nothing implemented.

Changes since the review. The branch was rebased on 2026-09-05 onto `bdc2918` (0.1.0-beta.2). Two changes landed in between and touched files this report cites. The About window overhaul (PR #45) took `sunlit-app/src/about.rs` from 105 to 705 lines and 3 to 9 tests, adding a markdown block parser and a URL opener; `ui/main.slint` from 1644 to 1868 lines, where `AboutWindow` grew from 41 to 250 lines and an `AboutBlock` struct was added near the top, so every line inside `MainWindow` moved down by 15 while `MainWindow` itself is unchanged; and `tests/slint_ui.rs` from 1165 to 1550 lines and 45 to 53 tests, all of it added after line 1124. The target-directory fix took `sunlit-app/build.rs` from 23 to 93 lines (it now records the target triple) and set `debug = false` for dependencies in the root `Cargo.toml`. `docs/architecture.md` and `docs/roadmap.md` gained a few lines. `sunlit-core` is untouched apart from two comments removed from its `Cargo.toml`. The line numbers in this report and in the notes were moved to match. The changed code was not reviewed, and the counts in section 3 are those of `19312ba`.
...
| `sunlit-app/ui/main.slint` | 1644 (1868 after PR #45) | | | one `MainWindow`; 832 lines are the Advanced section, which Slint cannot factor without adding lines |
...
| `sunlit-app/tests/slint_ui.rs` | 1165 (1550 after PR #45) | | | 45 tests (53 after PR #45), about 16 redundant |
...
Documentation drift in `docs/` itself: `architecture.md:153` still describes the two-slot wallpaper naming that `sweep_legacy_files` exists to delete; `architecture.md:80` lists the file-backed slots without the Milky Way; `roadmap.md:84` says seven of nine golden cases fail on Metal when it is fourteen of eighteen; `testing.md:35` says the golden suite holds `GPU_SERIAL` when it uses its own `ENGINE` mutex; `xtask/src/commands/e2e.rs:44` says the desktop e2e run is "well under a minute"; and the "adding a shader parameter" checklist exists in `CLAUDE.md`, `rendering.md:60` and `params.rs:1-8` and undercounts (section 4.4).
...
| `ui/main.slint` | `widgets.slint` (lines 3-210) and `tray.slint`; leave the 832-line Advanced section where it is, because Slint has no way to hand a component a group of two-way bindings and the split would add lines | app B5 |
...
| Gamma slider curve | `params.rs:430` and inlined twice in `main.slint:1399, 1425` with the constants baked in | change `GAMMA_MIN` and the labels lie with no test failing |
...
- Dragging the auto-refresh interval slider does a config-file read, serialize, write and rename per tick on the UI thread (`main.slint:431-439`, `main.rs:553-580`), plus one `SetAutoRefresh` command per pixel.
...
Each file is one reviewer's full report for its area, with the classification of every long comment block, the split seams with line ranges, the visibility grep results, and the per-test verdicts. Line numbers refer to commit `19312ba`.
