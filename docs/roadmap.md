...
- [x] Desktop e2e in local VMs: `cargo xtask` orchestrates a Windows guest on Hyper-V and a Linux guest on QEMU, builds the golden images with Packer, and runs the suite in the guest's console session. Phase 3, see [plans/2026-08-19-phase3-vm-orchestration-plan.md](plans/2026-08-19-phase3-vm-orchestration-plan.md) and [vm-setup.md](vm-setup.md). Implemented and unit-tested end to end. Live on a Windows host for the Linux guest; the Windows guest is blocked on a QEMU/WHPX fault that makes the installer's first reboot unsurvivable (see deviation 25 in the plan and the troubleshooting section of vm-setup.md). Either build that image on a Linux host or add a `hyperv-iso` template.
