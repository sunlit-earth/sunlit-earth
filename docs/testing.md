...
- Publishing needs a tag *and* an intention. A tag push publishes; a dispatch publishes only on a tag ref with `publish=true`, so a dispatch on a tag with `publish=false` builds and verifies without touching the release that tag already has; and a dispatch from a branch fails at the first step with the reason rather than handing the release action a branch name to tag.
- Cache keys are `ci-linux` / `ci-windows` / `ci-macos` for the test profile and `release-<platform>` for the release one, because release and test artifacts share nothing and a mixed cache evicts itself. `macos-build.yml` takes whichever its mode is in: `check` deliberately shares `ci-macos`, since it compiles the same tree in the same profile as the macOS test job, and `build` and `build-and-e2e` take `release-macos`, since what they compile is the release profile `release.yml` already has a warm cache of. The Windows test key is spelled out rather than derived from the runner label so it kept the key it had before the matrix.
