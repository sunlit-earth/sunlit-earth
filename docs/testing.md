...
- **The macOS job is red, on one engine case.** As of 2026-09-07 every target passes on `macos-latest` except `stars::zero_star_intensity_leaves_catalog_pixels_at_the_clear_color`, which failed on one covered pixel moving two of 255. The revision that answers it holds a covered pixel to the golden suite's outlier bound, and it has not run on Metal: the run's macOS job budget was spent, so whether the job is green now is a prediction. The soak case is the other one to watch there: it failed once in two runs on the warm-up boundary rather than on a leak, and the roadmap has both samples.
