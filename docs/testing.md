...
- Publishing needs a tag. The `publish` job runs only on a `refs/tags/` ref, and a dispatch asking for `publish=true` from a branch fails at the first step with the reason rather than handing the release action a branch name to tag. The input remains what it is used for, which is the dry run.
