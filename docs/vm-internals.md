...
cargo xtask vm up <image> [--desktop <kde|gnome|xfce|cinnamon|sway|i3>] [--session-type <x11|wayland>] [--screens <n>]   # An interactive guest, with the current binaries in it
...
cargo xtask e2e --target <host|windows|linux> [--keep] [--allow-expired-image] [--desktop <d>] [--session-type <t>] [--screens <n>]
...
| `linux` | base | Debian 13 with four desktops, sway and i3: the e2e guest |
...
binary, `README.txt` from `assets/readme/`, `textures/` with the four JXL assets, `LICENSE`, and
...
The Linux guest is Debian 13 with four desktops installed side by side, KDE and GNOME with a Wayland session as well as an X11 one, plus sway on Wayland and i3 on X11, and nothing in the image decides which one a boot logs into. `provider::desktop` is the host half. `Desktop::session(SessionType)` maps the `--desktop` and `--session-type` pair onto the `.desktop` basename sddm's `[Autologin] Session=` wants (`plasmax11`, `gnome-xorg`, `xfce`, `cinnamon` and `i3` on X11, `plasma`, `gnome-wayland` and `sway` on Wayland, few of which are the names one would guess) and answers `None` for XFCE, Cinnamon and i3 on Wayland and sway on X11; `Desktop::default_session_type` gives sway Wayland when `--session-type` is not given and everything else X11; `Login` is a pair that has a session, and `fw_cfg_args` puts its basename on QEMU's command line as `-fw_cfg name=opt/sunlit/desktop,string=<session>`. The `opt/` prefix is required; QEMU refuses anything outside it. The guest never learns there are two axes: it receives one basename, exactly as it did when there were four. In the guest, a oneshot unit ordered before `display-manager.service` reads `/sys/firmware/qemu_fw_cfg/by_name/opt/sunlit/desktop/raw`, checks the value against its own allowlist of the same eight names, looks the name up in `/usr/share/wayland-sessions` or `/usr/share/xsessions` as the name requires, and writes sddm's autologin drop-in. `the_guest_accepts_exactly_the_sessions_the_host_can_ask_for` reads both lists and compares them, and checks each name against the directory its type belongs in, since nothing else connects the two: a name on one side and not the other is a boot that silently falls back to Plasma while `vm status` says otherwise. fw_cfg rather than SMBIOS OEM strings, which was the other candidate: the fw_cfg device is ACPI-enumerated so its module loads itself early and the value is a file in sysfs, while mainline exports no per-string sysfs interface for SMBIOS type 11 at all. The chosen desktop and session type go into `RunState`, so they survive the command that chose them and `vm status` can name them; `--desktop` and `--session-type` against the Windows guest are refused rather than ignored, because a run whose flag did nothing is a run whose results are about a desktop nobody chose.

The Wayland names are chosen around sddm issue #837: autologin resolves `Session=` by basename and looks in the X11 directory first, and GNOME ships `gnome.desktop` in both, so `Session=gnome` starts GNOME on Xorg. `gnome-wayland.desktop` is the same session under a name only the Wayland directory has. `desktop.sh` asserts at build time that each Wayland name exists under `wayland-sessions` and does not exist under `xsessions`, so a later package that adds a colliding file fails the build and not a boot. `DisplayServer=x11` in sddm's configuration stays, because it governs the greeter only, and autologin never shows it.

Because that resolution is a silent one, every boot of the Linux image then proves the session it got. `Session::check_login` reads `session.env` after the ready marker and `Login::check_session_env` compares its `XDG_SESSION_TYPE` and `XDG_CURRENT_DESKTOP` with the login in the record, or with Plasma on X11 for a boot that named none; a mismatch fails the boot with both named. `XDG_CURRENT_DESKTOP` is split on `:` and compared per entry, because a distribution may prefix its own name. The session type is recorded in `RunState::session_type` beside the desktop, absent for X11, so a record written before the field reads as the X11 guest it was.

`session.env` gains `WAYLAND_DISPLAY` only when the session has one, so an X11 session's file is byte for byte what it was. With the line present everything that sources the file, the job, the hand-over launcher and `vm ssh`, starts the app as a Wayland client. A Wayland session's autostart environment was measured on the old image before any of this was written, by editing the marker to dump its environment and restarting sddm: GNOME 48 under sddm had `WAYLAND_DISPLAY=wayland-0`, `DISPLAY=:0` and `XAUTHORITY=/run/user/1000/.mutter-Xwaylandauth.<random>` when the marker ran, Plasma 6.3 the same with its own `xauth_<random>`, so neither needs the marker to wait for anything. `xhost` stays in the marker and does nothing under either compositor, which already grants the local user.

Two things are refused under Wayland rather than attempted, both before anything is built. More than one screen, in `vm::BootRequest::check`, because `place_screens_command` is `xrandr --right-of` and `map_pointer_command` is `xinput --map-to-output`, neither of which moves a Wayland compositor's outputs; porting them means `kscreen-doctor` and Mutter's `ApplyMonitorsConfig`, which is the roadmap's native display query rather than a patch here. And XFCE or Cinnamon, whose Wayland sessions are experimental in this release. sway and i3 start no XDG autostart entries, so the session marker that writes `session.env` is started from `/etc/sway/config.d/50-sunlit-e2e.conf` and from the test account's `~/.config/i3/config`, which is `/etc/i3/config` without its `i3-config-wizard` line, so no first-run dialog appears over the session.

Debian 13 rather than a current Ubuntu, and that choice has a shelf life. It is the only current base where Plasma, GNOME and XFCE all have a first-class X11 session at once: GNOME 50 removed X11 upstream in March 2026 and Ubuntu 25.10 had already dropped the GNOME Xorg session, while trixie froze on GNOME 48 and Plasma 6.3. That insulates the image for trixie's support window (full to 2028-08, LTS to 2030-06) and no longer. X11 stays the default session because the layout cases and `--screens` are `xrandr` commands; Plasma and GNOME on Wayland are a flag away in the same image, which is also what a base without a GNOME X11 session would leave.
...
A guest asked for more than one screen (`vm up --screens <n>`, up to `vm::MAX_SCREENS`) is given that display device as JSON instead, because the property that makes the extra screens work is a list and `-device` takes a list in no other form. Three things have to line up and only two of them are obvious. `max_outputs` sets the number of scanouts, which is how many connectors the guest's DRM driver creates. One `-vnc` server per head, each carrying `display=<device id>` *and* `head=<n>`, makes each console its own screen rather than another view of the first; QEMU accepts as many VNC servers as it is given, and `RunState::vnc_heads` records where they all are, with `vnc` staying the first so everything that wants one console keeps the field it read before. `head=` alone is the trap and cost a round of this: it parses, the guest starts, and both servers serve the first screen, because `vnc_display_open` looks a console up only when `display` names a device. A device id that is not there is refused at startup, so the id is checked; leaving it out is not. The third is an `outputs` entry per screen carrying its own `xres`/`yres`, and without it the guest sees a second connector that reports itself `disconnected`: virtio-gpu enables output 0 when the device is realized and leaves the rest to whatever UI eventually says how big it is, which for VNC means a client sending `SetDesktopSize` and not merely connecting. Measured in the Debian guest on 2026-08-30: `max_outputs` alone gives `card0-Virtual-2` as `disconnected` and a one-screen session; with the `outputs` list both connectors read `connected` and the Plasma session comes up 3840x1080 with the second screen already to the right of the first. The device also takes an id there (`qemu::DISPLAY_DEVICE_ID`), which is the only way to name a screen other than the first to QMP's `screendump`.

How many screens a guest has is a property of the boot, so it is recorded like the desktop is: `vm::screens_for` decides whether the image may be asked at all (only the Debian desktop guest, since the Windows adapter has one head under either hypervisor), `boot` writes it to the record before the machine is started, and the provider builds the command line from the record. `e2e` and `vm up` ask the same questions, through `BootRequest::check`, before they build anything, because `boot` is on the far side of a compile that takes minutes and a usage error there arrives too late to be worth much. The extra consoles are the one thing not picked when the record is created, because the count is not known then; `qemu::fill_in_consoles` picks them at the top of `start`, walking from one past the last rather than from the base so two screens cannot land on one port, and the record is written again immediately afterwards. After the session is ready, `vm::place_screens` runs `xrandr --output <b> --auto --right-of <a>` over SSH through the session's own `session.env`, then maps the pointer and reports both. The pointer is the half QEMU does not do: it scales a head's coordinates onto the whole absolute range using that head's width and adds no offset for the head's place in the guest's desktop, so one tablet on a two-screen guest doubles every x. A tablet per head bound with `display=`/`head=` is the obvious answer and does not work: unlike `-vnc`, an input device accepts a `display=` naming nothing at all, and with one tablet bound to each head, pointer events from both VNC servers were measured arriving at the same device by watching the guest's `/dev/input/event*`. So there is one tablet, and `vm::map_pointer_command` maps it to the primary output with `xinput --map-to-output`, which makes that screen's window exact and leaves the others to look at. It needs the `xinput` package; a guest without it says so in one line and boots anyway. And it reports what the session actually has: X's default is every output at the origin, a count alone would call two stacked screens a success, and neither a short count nor a stacked pair ends the boot, since the guest is up either way and the answer is in front of whoever looks at it.
