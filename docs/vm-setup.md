...
The images live outside the repository, in `%LOCALAPPDATA%\SunlitEarth\vm` on Windows and `~/.local/share/SunlitEarth/vm` on Linux. Set `SUNLIT_EARTH_VM_DIR` to put them somewhere else, on a bigger disk for instance. Measured on this host: the Windows image is 14.9 GiB of qcow2 plus 19.3 GiB of VHDX, the Debian image 4.0 GiB, and `linux-builder` 3.1 GiB, with the Windows download another 6.6 GB. Budget 60 to 70 GB for all four images plus their overlays and the media, and remember that an overlay grows with what a run writes into it: a release build's `target/release` lives in one.
...
**`ssh` warns that the remote host identification has changed.** Expected on the Linux guest, on any run: that image ships with no host keys, because one identity shared by every overlay is worse than none, so a first-boot unit generates a fresh set and every boot is a new host. The Windows image keeps the keys it was built with, so there the warning means the image was rebuilt or replaced. The xtask keeps its own known-hosts file at `<store>/ssh/known_hosts` rather than touching yours, and a build deletes it whether it produced an image or failed: a build that failed after its guest answered on SSH has already written an entry that later runs would warn about. Delete that file to silence the warning; nothing else needs it, and public-key authentication is unaffected either way.
