...
**`ssh` warns that the remote host identification has changed.** A guest's host keys live on the golden image, so they change when the image is rebuilt and stay the same across every run in between. The xtask keeps its own known-hosts file at `<store>/ssh/known_hosts` rather than touching yours, and a finished build deletes it for that reason. If the warning appears anyway, for instance after an image was replaced by hand, delete that file; nothing else needs it, and public-key authentication was never affected.
