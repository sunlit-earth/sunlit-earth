...
**A Windows build reaches "Start PXE over IPv4" and then hangs.** The keypress that answers "Press any key to boot from CD or DVD" was missed, so the firmware fell through every other boot option. That key does not come from Packer: its `boot_command` is empty, because its keystrokes were measured not to reach the guest, and `build-image` presses the key over QMP instead. The command prints how many presses it sent. If it printed none, or could not reach the monitor, something else is on port 4445; if it sent them and the prompt still timed out, the boot is slower than the window and `BOOT_KEY_PRESSES` in `build_image.rs` is the number to raise.
