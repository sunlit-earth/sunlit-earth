...
pub(crate) const GRID_TEX_WIDTH: u32 = 2048;
pub(crate) const GRID_TEX_HEIGHT: u32 = 1024;
...
/// Create the bind group layout shared by all texture bind groups.
...
/// Create the shared trilinear + anisotropic sampler used by all textures.
...
/// Create and load the combined blend + sphere shader module.
...
/// Generate sphere mesh and create vertex/index buffers.
///
/// Returns `(vertex_buffer, index_buffer, index_count)`.
pub(crate) fn create_sphere_buffers(device: &wgpu::Device) -> (wgpu::Buffer, wgpu::Buffer, u32) {
...
/// Create a 1x1 black dummy texture used as the night texture placeholder
/// in single-texture bind groups (Grid, Day, Night modes).
pub(crate) fn create_dummy_texture(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
) -> wgpu::TextureView {
...
/// Create the pipeline layout with a single bind group.
...
pub(crate) fn create_cloud_pipeline(
    device: &wgpu::Device,
    pipeline_layout: &wgpu::PipelineLayout,
    shader: &wgpu::ShaderModule,
    sample_count: u32,
...
pub(crate) fn create_render_textures(
    device: &wgpu::Device,
    width: u32,
    height: u32,
    sample_count: u32,
